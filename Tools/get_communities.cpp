			...
			if(vf2_subgraph_iso(graph1, graph2, callback)){
			    ...
			    std::chrono::duration<double> elapsed = finish - start;
			 	...
			 	if(elapsed.count() > 2.0){
			 		...
			 		fout << quant_vertices_per_community[i] << " " << communities_edges[i].size() << endl;
					print_adj_matrix(communities_edges[i], fout);
					fout << endl;
					fout << quant_vertices_per_community[hash_table[{n, m}][j]] << " " << communities_edges[hash_table[{n, m}][j]].size() << endl;
